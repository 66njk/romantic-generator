...
const Post = ({ id, isLatest, title, date }) => {
  return (
    <article className={`flex flex-col w-full h-auto border-b border-gray-900 dark:border-white dark:border-opacity-10 border-opacity-10 pb-6 mb-10 ${isLatest ? 'md:col-span-3 md:grid md:grid-cols-2 md:gap-x-12' : ''}`}>
      ...
      <section className={`flex flex-col justify-between h-28 ${isLatest ? 'md:h-full' : ''}`}>
        {/* 标题 */}
        <header>
          <Link href={`/posts/${id}`}>
            <h2 className={`text-2xl font-semibold text-gray-900 dark:text-white overflow-ellipsis line-clamp-2 text-clickable ${isLatest ? 'md:text-5xl md:leading-snug' : ''}`}>
              <span>{title}</span>
            </h2>
          </Link>
        </header>
        ...
        <div className={`max-w-cmw min-h-screen mx-auto my-8 px-4`}>
          <section className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-x-6 w-full h-auto'>
            {allPostsData.map(({ id, date, title }, index) => (
              <Post id={id} title={title} date={date} isLatest={index === 0 ? true : false} key={id} />
            ))}
          </section>
        </div>
      </Layout>
